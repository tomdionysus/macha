...
constexpr uint32_t frame_magic = 0x4d434834; // "MCH4"
constexpr size_t max_frame_size = 128 * 1024 * 1024;
constexpr size_t health_worker_count = 2;
constexpr size_t control_worker_count = 6;
...
Bytes session_info(std::span<const uint8_t> transcript, const NodeId& client,
    ...
    Writer writer;
    writer.string("macha/session/v4");
...
RpcLane rpc_lane(MessageType type) {
    if (type == MessageType::ping)
        return RpcLane::health;
    if (type == MessageType::get_object || type == MessageType::put_object)
        return RpcLane::data;
    return RpcLane::control;
}

const char* rpc_lane_name(RpcLane lane) noexcept {
    switch (lane) {
    case RpcLane::health:
        return "health";
...
namespace {
RpcLane decode_rpc_lane(uint8_t value) {
    switch (value) {
    case static_cast<uint8_t>(RpcLane::health):
        return RpcLane::health;
    case static_cast<uint8_t>(RpcLane::control):
        return RpcLane::control;
    case static_cast<uint8_t>(RpcLane::data):
        return RpcLane::data;
    default:
        throw std::runtime_error("bad transport lane");
    }
}
} // namespace

SecureChannel::SecureChannel(int fd, ClusterKeys keys, NodeInfo local)
    : fd_(fd), keys_(keys), local_(std::move(local)) {
    socket_options(fd_);
}
...
void SecureChannel::shutdown() {
    std::lock_guard lock(close_mutex_);
    if (fd_ >= 0 && !shutdown_) {
        // Wake blocked readers/writers without closing/recycling the descriptor
        // underneath them. The descriptor is closed only after the channel's
        // owning threads have joined and SecureChannel is destroyed.
        ::shutdown(fd_, SHUT_RDWR);
        shutdown_ = true;
    }
}

NodeInfo SecureChannel::client_handshake(RpcLane lane) {
    ...
    hello_writer.u8(static_cast<uint8_t>(lane));
    ...
    envelope.fixed(hmac_sha256(keys_.auth, label("client/v4", hello)));
    ...
    auto authenticated = label("server/v4", hello);
    ...
    if (decode_rpc_lane(reader.u8()) != lane || reader.fixed<16>() != keys_.cluster_id)
        throw std::runtime_error("wrong cluster/protocol lane");
...
NodeInfo SecureChannel::server_handshake(const std::string& remote_host, RpcLane* negotiated_lane) {
    ...
    if (!constant_time_equal(remote_mac, hmac_sha256(keys_.auth, label("client/v4", hello))))
        throw std::runtime_error("client auth failed");

    Reader reader(hello);
    if (reader.u16() != protocol_version)
        throw std::runtime_error("wrong cluster/protocol");
    const auto lane = decode_rpc_lane(reader.u8());
    ...
    ack_writer.u8(static_cast<uint8_t>(lane));
    ...
    ready_ = true;
    if (negotiated_lane)
        *negotiated_lane = lane;
    return peer;
}

void SecureChannel::send(uint64_t request_id, const RpcMessage& message,
                         const std::function<void(size_t)>& progress) {
    if (!ready_ || message.payload.size() > max_frame_size)
        throw std::runtime_error("bad secure send");

    Writer header;
    header.u32(frame_magic);
    header.u16(static_cast<uint16_t>(message.type));
    header.u16(0);
    header.u32(static_cast<uint32_t>(message.payload.size()));
    header.u64(++tx_counter_);
    header.u64(request_id);
    auto sealed = aes_gcm_seal(tx_, message.payload, header.data());
...
RpcFrame SecureChannel::receive(const std::function<void(uint64_t, size_t)>& progress) {
    ...
    auto type = static_cast<MessageType>(header.u16());
    if (header.u16() != 0)
    ...
    if (size > max_frame_size || counter != rx_counter_ + 1)
    ...
    rx_counter_ = counter;
    return {request_id, {type, std::move(payload)}};
}
...
AsyncRpc::AsyncRpc(AsyncRpc&& other) noexcept
    : future_(std::move(other.future_)), cancel_(std::move(other.cancel_)),
      abort_(std::move(other.abort_)), idle_(std::move(other.idle_)) {
    // std::function's moved-from state is valid but unspecified. AsyncRpc owns
    // cancellation, so a moved-from wrapper must be explicitly disarmed; its
    // destructor must never cancel the request now owned by *this.
    other.cancel_ = {};
    other.abort_ = {};
    other.idle_ = {};
}
...
AsyncRpc& AsyncRpc::operator=(AsyncRpc&& other) noexcept {
    ...
    // Replacing an in-flight AsyncRpc relinquishes that request exactly as its
    // destructor would. Then transfer the new request and explicitly disarm the
    // source for the same reason as the move constructor above.
    cancel();
...
class RpcClient::PeerConnection : public std::enable_shared_from_this<RpcClient::PeerConnection> {
    ...
    void queue_frame(uint64_t request_id, const RpcMessage& message) {
        {
            ...
            outbound_.push_back(
                {request_id, {message.type, Bytes(message.payload.begin(), message.payload.end())}});
        }
        outbound_cv_.notify_one();
    }

    void dispatch_request(RpcFrame frame) {
        if (rpc_lane(frame.message.type) != lane_)
            throw std::runtime_error("RPC request used the wrong transport lane");
        if (!inbound_handler_)
        ...
        try {
            inbound_handler_(peer_, std::move(frame), [weak, id](const RpcMessage& reply) {
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    channel_.send(item.request_id, item.message,
                                  [this, id = item.request_id](size_t) { touch(id); });
                } catch (...) {
                    --writes_active_;
                    throw;
                }
    ...
    void reader_loop(std::stop_token stop) {
        try {
            while (!stop.stop_requested()) {
                auto frame = channel_.receive([this](uint64_t request_id, size_t) {
                ...
                // The dialler owns odd request IDs. Even IDs arriving here are
                // requests originated by the TCP acceptor on this same session.
                ...
                std::shared_ptr<Pending> pending;
                {
                    ...
                    if (found == pending_.end())
                        continue; // Cancelled request result.
  ...
  public:
    PeerConnection(int fd, ClusterKeys keys, NodeInfo local, RpcLane lane,
        ...
        : channel_(fd, keys, std::move(local)), lane_(lane),
        ...
        peer_ = channel_.client_handshake(lane_);
    ...
    RpcLane lane() const noexcept { return lane_; }
    ...
    AsyncRpc call(MessageType type, std::span<const uint8_t> payload) {
        ...
        {
            // Retirement's FIFO marker is only meaningful if admitting a new
            // RPC and beginning retirement are mutually exclusive. A call
            // admitted here is fully queued before retire() can enqueue its
            // marker; a later call sees retiring_ and is rejected.
            ...
            try {
                queue_frame(id, {type, Bytes(payload.begin(), payload.end())});
            } catch (...) {
                std::lock_guard lock(pending_mutex_);
                pending_.erase(id);
                throw;
        ...
        return AsyncRpc(std::move(future), std::move(cancel), std::move(abort), std::move(idle));
    }

    void notify(const RpcMessage& message) {
        if (!usable())
            return;
        // Notifications preserve the old synchronous broadcast semantics: the
        // call returns once the frame has been written to the socket. Serialise
        // with the writer thread rather than waiting on its queue; callers may
        // hold higher-level metadata/RPC locks while publishing invalidations.
        std::lock_guard lock(send_mutex_);
        if (usable())
            channel_.send(0, message);
    }
    ...
    void retire() {
        {
            ...
            if (retiring_.compare_exchange_strong(expected, true)) {
                try {
                    // FIFO behind every RPC admitted before retirement. Once
                    // this marker is queued, no later RPC can be admitted.
                    queue_frame(0, {MessageType::session_retire, {}});
                } catch (...) {
                    close();
                    return;
                }
...
RpcClient::RpcClient(ClusterKeys keys, std::function<NodeInfo()> local,
                     ...
                     std::chrono::milliseconds dead_after)
    : keys_(keys), local_(std::move(local)), peer_observer_(std::move(peer_observer)),
      ...
      heartbeat_(heartbeat), dead_after_(dead_after) {
...
std::string RpcClient::health_key(const Endpoint& endpoint, RpcLane lane) {
    return endpoint_key(endpoint) + '#' + rpc_lane_name(lane);
}

std::string RpcClient::peer_key(const NodeId& peer, RpcLane lane) {
    return to_string(peer) + '#' + rpc_lane_name(lane);
}
...
void RpcClient::dispatch_inbound(const NodeInfo& peer, RpcFrame frame, InboundReply reply) {
    // Serialise replacement with dispatch so RpcServer::stop() cannot leave a
    // copied callback referring to a server that is already tearing down.
    std::lock_guard lock(inbound_mutex_);
    if (!inbound_handler_)
        throw std::runtime_error("no inbound RPC handler installed");
...
void RpcClient::reap_retired() {
    ...
    // Destruction can report final request failures through observe_result(),
    // so references are deliberately released outside mutex_.
    reap.clear();
}

void RpcClient::reconcile_locked(const NodeId& peer, RpcLane lane,
                                 std::vector<std::function<void()>>& retire) {
    const auto k = peer_key(peer, lane);
    ...
    // Both nodes compute the same physical winner: the lower NodeId is the TCP
    // dialler and the higher NodeId is the acceptor.
    if (local_().id < peer) {
...
void RpcClient::unregister_inbound(const NodeId& peer, RpcLane lane,
                                   const std::array<uint8_t, 32>& session_id) {
    std::lock_guard lock(mutex_);
    auto found = inbound_routes_.find(peer_key(peer, lane));
...
RpcClient::connection(const Endpoint& endpoint, RpcLane lane, const NodeId* expected,
                      NodeId* actual) {
    reap_retired();
    const auto retry_key = health_key(endpoint, lane);
    std::optional<NodeId> known;
    {
        ...
        if (known) {
            if (actual)
                *actual = *known;
            auto found = connections_.find(peer_key(*known, lane));
            if (found != connections_.end() && found->second && found->second->usable()) {
                ++connections_reused_;
                return found->second;
            }
            auto inbound = inbound_routes_.find(peer_key(*known, lane));
    ...
    try {
        ...
        auto fresh = std::make_shared<PeerConnection>(
            fd, keys_, local_(), lane, peer_observer_, metadata_observer_,
        ...
        bool installed = false;
        {
            ...
            const auto k = peer_key(fresh->peer().id, lane);
        ...
        if (installed && winner == fresh) {
            Log::info("node connection outbound peer=" + to_string(fresh->peer().id).substr(0, 12) +
                      " endpoint=" + endpoint_key(endpoint) + " lane=" + lane_name(lane));
        }
        return winner;
    } catch (...) {
...
AsyncRpc RpcClient::call_async_known(const Endpoint& endpoint, const NodeId* expected,
    ...
    auto try_existing = [&](const NodeId& peer) -> std::optional<AsyncRpc> {
        ...
        if (outbound) {
            ++connections_reused_;
            try {
                return outbound->call(type, payload);
    ...
    NodeId actual{};
    auto outbound = connection(endpoint, lane, expected, &actual);
...
AsyncRpc RpcClient::call_async(const Endpoint& endpoint, MessageType type,
    ...
    return call_async_known(endpoint, nullptr, type, payload);
}

AsyncRpc RpcClient::call_async(const NodeInfo& node, MessageType type,
                               std::span<const uint8_t> payload) {
    Endpoint endpoint{node.host, node.port};
    return call_async_known(endpoint, &node.id, type, payload);
}
...
RpcReply RpcClient::call(const Endpoint& endpoint, MessageType type,
    ...
    const auto lane = lane_for(type);
    auto async = call_async(endpoint, type, payload);
    ...
    while (async.wait_for(stall_notice) != std::future_status::ready) {
        ...
        if (idle >= stall_notice) {
            Log::debug(std::string("RPC stalled (") + lane_name(lane) + ") peer=" +
                       endpoint_key(endpoint) + " no_progress_ms=" + std::to_string(idle.count()) +
...
RpcReply RpcClient::call(const NodeInfo& node, MessageType type,
                         ...
                         std::chrono::milliseconds stall_notice) {
    const auto lane = lane_for(type);
    auto async = call_async(node, type, payload);
    if (stall_notice.count() <= 0)
        return async.get();
...
void RpcClient::close_endpoint(const Endpoint& endpoint, const std::string& reason) {
    ...
    std::optional<NodeId> peer;
    {
        ...
        for (auto lane : {RpcLane::health, RpcLane::control, RpcLane::data}) {
            const auto k = peer_key(*peer, lane);
            auto out = connections_.find(k);
    ...
    while (!stop.stop_requested()) {
        ...
        std::map<NodeId, Endpoint> active_peers;
        {
            ...
            for (const auto& [base, endpoint] : endpoints_) {
                ...
                bool active = false;
                for (auto lane : {RpcLane::control, RpcLane::data}) {
                    const auto k = peer_key(peer, lane);
                    auto out = connections_.find(k);
                    if (out != connections_.end() && out->second && out->second->usable()) {
                        active = true;
                    ...
                    auto in = inbound_routes_.find(k);
                    if (in != inbound_routes_.end() && in->second.usable &&
                        in->second.usable()) {
                        active = true;
                        break;
                    }
...
RpcStats RpcClient::stats() const {
    ...
    auto count = [&](RpcLane lane) {
        ++stats.canonical_connections;
        switch (lane) {
        case RpcLane::health:
            ++stats.canonical_health;
            break;
        case RpcLane::control:
            ++stats.canonical_control;
            break;
        case RpcLane::data:
...
void RpcClient::broadcast(const RpcMessage& message) {
    const auto lane = rpc_lane(message.type);
    std::vector<std::shared_ptr<PeerConnection>> outbound;
    std::vector<std::function<void(const RpcMessage&)>> inbound;
    {
        std::lock_guard lock(mutex_);
        for (const auto& [_, connection] : connections_)
            if (connection && connection->usable() && connection->lane() == lane)
                outbound.push_back(connection);
        for (const auto& [_, route] : inbound_routes_)
            if (route.lane == lane && route.usable && route.usable() && route.notify)
...
struct RpcServer::Session : public std::enable_shared_from_this<RpcServer::Session> {
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    channel->send(item.request_id, item.message, [this, id = item.request_id](size_t) {
                        if (!id || (id & 1U))
                            return;
                        std::lock_guard lock(pending_mutex);
    ...
    AsyncRpc call(MessageType type, std::span<const uint8_t> payload) {
        if (!lane_known.load() || rpc_lane(type) != lane)
            throw std::runtime_error("RPC request used the wrong transport lane");
...
RpcServer::RequestClass RpcServer::request_class(MessageType type) {
    if (rpc_lane(type) == RpcLane::health)
        return RequestClass::health;
    if (rpc_lane(type) == RpcLane::data)
...
std::deque<RpcServer::RequestJob>& RpcServer::queue(RequestClass cls) {
    switch (cls) {
    case RequestClass::health:
        return health_requests_;
    case RequestClass::control:
        return control_requests_;
    case RequestClass::data:
        return data_requests_;
    }
    return control_requests_;
}
...
void RpcServer::enqueue_shared(const NodeInfo& peer, RpcFrame frame,
    ...
    const auto cls = request_class(frame.message.type);
    {
...
void RpcServer::start() {
    ...
    health_workers_.reserve(health_worker_count);
    control_workers_.reserve(control_worker_count);
    data_workers_.reserve(data_worker_count);
    for (size_t i = 0; i < health_worker_count; ++i)
        health_workers_.emplace_back(
            [this](std::stop_token stop) { worker_loop(stop, RequestClass::health); });
    for (size_t i = 0; i < control_worker_count; ++i)
        control_workers_.emplace_back(
            [this](std::stop_token stop) { worker_loop(stop, RequestClass::control); });
    for (size_t i = 0; i < data_worker_count; ++i)
        data_workers_.emplace_back(
            [this](std::stop_token stop) { worker_loop(stop, RequestClass::data); });
...
void RpcServer::stop() {
    ...
    for (auto* workers : {&health_workers_, &control_workers_, &data_workers_})
        for (auto& worker : *workers)
            worker.request_stop();
    request_cv_.notify_all();
    health_workers_.clear();
    ...
    {
        std::lock_guard lock(request_mutex_);
        for (auto* requests : {&health_requests_, &control_requests_, &data_requests_}) {
...
void RpcServer::accept_loop(std::stop_token stop) {
    while (!stop.stop_requested()) {
        ...
        session->channel = std::make_unique<SecureChannel>(client, keys_, local_);
        {
            std::lock_guard lock(sessions_mutex_);
...
void RpcServer::session_loop(Session* session) {
    try {
        session->peer = session->channel->server_handshake(session->remote_host, &session->lane);
        session->lane_known = true;
        session->ready = true;
        ...
        Log::info("node connection inbound peer=" +
                  to_string(session->peer.id).substr(0, 12) +
                  " remote=" + session->remote_host +
                  " advertised=" + session->peer.host + ':' +
                  std::to_string(session->peer.port) +
                  " lane=" + rpc_lane_name(session->lane));
        while (true) {
            auto frame = session->channel->receive([session](uint64_t request_id, size_t) {
                if (!request_id || (request_id & 1U))
                    return;
            ...
            // The acceptor owns even request IDs. Even frames arriving here are
            // replies to calls originated on this accepted session.
            if ((frame.request_id & 1U) == 0) {
            ...
            const auto cls = request_class(frame.message.type);
            const auto lane = rpc_lane(frame.message.type);
            if (session->lane != lane)
                throw std::runtime_error("RPC request changed transport lane");
            ...
            bool queued = false;
            {
                ...
                if (queue(cls).size() < max_pending_requests) {
                    ...
                    if (shared) {
                        ...
                        queue(cls).push_back(
                            {std::move(shared), session->peer, std::move(frame), {}});
                        queued = true;
                    }
                }
            }
    ...
    if (shared_client_ && session->lane_known.load())
        shared_client_->unregister_inbound(session->peer.id, session->lane,
...
void RpcServer::worker_loop(std::stop_token stop, RequestClass cls) {
    while (true) {
        RequestJob job;
        {
            std::unique_lock lock(request_mutex_);
            request_cv_.wait(lock, [&] {
                return stop.stop_requested() || !queue(cls).empty();
            });
            auto& requests = queue(cls);
            if (stop.stop_requested() && requests.empty())
                return;
        ...
        try {
            ...
            if (job.reply) {
                ...
                job.session->queue_frame(job.frame.request_id, reply);
            }
        } catch (const std::exception& error) {
