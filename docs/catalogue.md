...
Movie and TV filename recognition uses scored candidate inference rather than a single destructive parse. Candidate generators implement `MediaProbeCandidateGenerator` and emit a `MediaProbe`, score and human-readable evidence. The default pipeline currently includes filename-semantic, directory-structural, compact-title and legacy fallbacks. Strong evidence such as `S02E04`, an adjacent `(2003)` year, agreement with a `Season 2` directory, or a technical boundary such as `1080p`/`BluRay` outweighs weak release-name evidence. The scanner retains the best few distinct candidates and may try up to three provider interpretations within the existing provider request budget. Adding another naming convention is therefore a new generator, not another global regex that can damage unrelated filenames.
