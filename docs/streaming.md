...
`segment_timeout_ms` must stay under the client's time-to-first-byte deadline. A held request sends no bytes, so a client that gives up first never receives the `503` and takes its timeout path instead, which retries hard and then fails — strictly worse than not holding. For hls.js that deadline is `fragLoadPolicy.default.maxTimeToFirstByteMs` at 10s, not the deprecated and inert `fragLoadingTimeOut` at 20s, which is why the default here is 8000. A `503` that arrives promptly is retried sensibly instead: 6 attempts backing off 1s to 8s. `Retry-After` is sent because it is correct HTTP, but hls.js reads it only in its content-steering loader, so nothing here depends on it.

The global budget is small because `HttpServer` runs a fixed worker pool shared by every route, so a held request occupies a worker for the whole of its wait and an unbounded number of them would stop the node answering Status and control traffic. That is a property of the current runtime rather than of the design: under an async `HttpServer` a hold costs a continuation instead of a thread, the admission policy above is unchanged, only the waiting primitive differs, and `max_concurrent_holds` becomes a fairness and memory bound whose natural value is much larger.
