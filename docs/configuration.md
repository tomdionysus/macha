...
`commit_workers` is the maximum asynchronous extent-publication concurrency once the mount is genuinely quiet. Crash-recovered durable writes use an additional `recovery_commit_workers` ceiling (default two, and always effectively capped by `commit_workers`) so restart convergence cannot saturate a single storage backend merely because a large journal backlog exists. Live publications are selected ahead of recovery work. While any writable FUSE handle remains open, or any FUSE activity occurred within `publication_quiet_ms`, only `foreground_commit_workers` total publishers are admitted (default one). The default quiet period is five seconds so a short rsync inter-file gap cannot unleash the full publication fan-out. Final metadata commits are serialised even when several workers prepare immutable extents concurrently.
