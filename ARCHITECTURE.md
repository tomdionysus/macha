...
Each active peer pair can have two persistent authenticated bidirectional TCP lanes. `CONTROL` carries health, membership and other small control-plane RPCs. `DATA` carries object traffic plus metadata snapshots when they are explicitly classified as user-interactive or background work. The DATA lane is lazy and is opened when prioritised object or metadata traffic is first required. Separating the TCP sequence spaces prevents retransmission or kernel buffering of bulk payloads from head-of-line blocking liveness and membership traffic.
