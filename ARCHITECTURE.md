...
- by default, the FUSE operation journal and per-inode local write spools; both
  locations may be configured independently outside `state_path`;
...
`state_path` is mandatory and is never inferred from a storage backend. The FUSE spool is bulk temporary admission data and can be relocated with `fuse.spool_path`; its durable operation journal can separately be relocated with `fuse.operation_journal_path`.
