...
API catalogue reads are live but cached. A decoded catalogue object remains reusable while its immutable `catalogue_root` is current. Metadata generation notices invalidate that assumption immediately, while `metadata_cache_ms` bounds staleness if a notice is missed. The first API worker that observes invalidation performs a single-flight metadata/root convergence; concurrent readers reuse its result. If validation temporarily fails, a previously coherent immutable catalogue may still be served, with cached and known metadata generations exposed separately in catalogue status.
