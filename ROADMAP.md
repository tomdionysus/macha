...
0.10.2 fixes the measured macOS pathname-identity failure without migrating Macha namespace metadata. `readdir` presents stored names in Unicode Normalization Form D as required by macFUSE/Finder, while the in-memory namespace index resolves canonically-equivalent lookup paths back to the exact persisted key. New leaves use NFC beneath the actual persisted parent spelling, preserving compatibility with older NFC, NFD and mixed-component namespace state. This implementation does not require macFUSE's historical `norm_insensitive` mount option, which is not accepted by the libfuse3 stack used by the current macOS build. `log_level=all` records incoming path bytes and both stored/emitted dirent bytes for future platform-specific diagnosis.
